...
(** The carve infrastructure includes:
    - variables of type A annotated by multiplicities
...
(** Multiplicities: m_0 : available once; m_1 : used *)
Module lmult.
...
(** Of course, mult_eq is equivalent to =. Let's stick with mult_eq. *)
...
(** Note: The beluga lctx is length indexed, which makes lctx a dependent type
    and also somewhat complicates merge and everything else that works on lctx.
    Also, notice how binders are treated: they don't need to introduce them under a forall.

    Here was an original attempt, highlighting how the dependent type makes it a bit harder to write.
...
Inductive cx_eq n : lctx n -> lctx n -> Prop :=
| cx_refl : forall D, cx_eq n D D.
...
(** Update:
    ...
    Comment: could rewrite this to avoid foralls in each clause (what would that do?)
 *)
Inductive upd :
...
(** Exh: exhausted context
   Only harmless assumptions appear in D. For this particular type system (linear),
   recall an assumption is harmless if it is unavailable. Hence an exhausted context
   for this linear system is a context with
   all multiplicities set to 0 ('practically an empty context')
 *)
...
(** same_elts :
    ...
    Comment: last case in beluga is written: [same_elts (cons Δ X A _) (cons Δ' X A _)]. What does that
    placeholder "_" imply -- can they be different? Yes. Because we don't care about multiplicities.
    Could we use _ in Coq? Only in matches (pattern matching), as far as I am aware.
 *)
