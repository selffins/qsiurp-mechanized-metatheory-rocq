...
This is like modus ponens, HOWEVER, notice that we split our resources to prove =A -o B=, then prove =A=. This is like multiplicative rules: we need both =A -o B= and =A=, so we cannot use all of our resources to prove =A -o B= only or =A= only, hence the splitting.
